...
import MSconta.config.RabbitMQConfig;
...
public class RabbitMQConsumer {
    ...
    @RabbitListener(queues = RabbitMQConfig.QUEUE_CONTA_CRIAR)
    public void processarAprovacaoConta(AprovacaoDTO saga) {
        LOGGER.info("MS-CONTA: Recebido comando de APROVAÇÃO (Criar Conta) para CPF: {}", saga.cpf());
        try {
            AprovacaoDTO resultado = contaCUDService.criarContaAprovada(saga);

            if ("SUCESSO".equals(resultado.status())) {
                LOGGER.info("MS-CONTA: Conta criada com SUCESSO! Retornando para Orquestrador.");
            } else {
                LOGGER.error("MS-CONTA: Falha ao criar conta. Erro: {}", resultado.mensagemErro());
            }
            rabbitMQProducer.enviarRespostaSaga(resultado);

        } catch (Exception e) {
            e.printStackTrace();
            rabbitMQProducer.enviarRespostaSaga(saga.comErro(e.getMessage()));
        }
    }
    ...
    public void consumerSaqueDeposito(SaqueDepositoDTO data) {
        try {
            Optional<ContaR> optContaTemp = contaRRepository.findByNumConta(data.numConta());
    ...
    public void transferencia(TransferenciaDTO data) {
        try {
            Optional<ContaR> optContaTemp = contaRRepository.findByNumConta(data.numConta());
            Optional<ContaR> optContaTemp2 = contaRRepository.findByNumConta(data.numConta2());
